...
if ( 1 )	% parameters
	...
	SubjectDrummerNumber = 9:6;
	SubjectOrder = [ 1 2 3 5 6 7 8 9 10 11 6 13 ];
	SubjectId = [ 3 4 5 6 13 14 15 17 18 24 25 26 ];
...
% level of control
% for each movement
XT = zeros(6*numel(listSubjects), 3*numel(listSubjects) + numel(listSubjects));
YT = zeros(6*numel(listSubjects), 1);
...
for k=1:numel(listSubjects),
    ...
    for p=1:6, % six cases
        ...
        data = nmGet(dbs, {'nChoice','tMvmtOffset', 'MT=tEnterTarget-tMvmtOnset','PV=nPeakVelocity','tt=trialTypeNumber','E=nYourError','nB=nBlock','nT=nTrial','myE=nYourError'});

        Y = [ Y; nansum(data.nChoice==1)/(nansum(data.nChoice==1) + nansum(data.nChoice==2))];
        X = [ X; listE(p) listT(p) ];
    ...
    YT((k-1)*6+1:k*6) = YZ;
    XT((k-1)*6+1:k*6, (k-1)*3+1:k*3 ) = XZ;
    XT((k-1)*6+1:k*6, 3*numel(listSubjects)+k) = ones(6,1);
...
betas1(1,:) = b1(3*numel(listSubjects)+1:end);
betas1(2:4,:) = reshape(b1(1:3*numel(listSubjects)),3,numel(listSubjects));
...
g1 = figure;
barweb(nanmean(betas1,2), nanstd(betas1,0,2)/sqrt(numel(listSubjects)), 1, [], [], {}, 'Beta Coeffs. (Prob)', bone, [], {} );
set(g1,'color',[1 1 1]);
print(g1,'-dpdf','-r600',sprintf('Beta-ProbGroup.pdf'));    
...
%     i6x = find(X1==2 & X2==1 & X3>54 & X45==1);
...
%     end
% 
%     %betasG2(:,:,m,r) = reshape(betasG,6,numel(listSubjects));
%     % assess significance
